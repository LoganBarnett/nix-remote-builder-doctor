...
impl TestResult {
  ...
  /// Placeholder `Fail` result returned by tests that have not been
  /// implemented yet.  Named `not_implemented` rather than `default`
  /// to avoid suggesting an association with the `Default` trait.
  pub fn not_implemented(
    context: &MachineTestContext,
    test_name: &str,
  ) -> TestResult {
