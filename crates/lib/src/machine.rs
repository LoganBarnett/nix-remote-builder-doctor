//! Build-machine configuration as Nix's remote build system sees it.
...
fn parse_field_string(
  ...
  field_candidate
    .ok_or(AppError::MachinesEntryMissingFieldError(field_name.to_string()))
    .map(|x| x.to_string())
}
...
fn parse_field_u32(
  ...
  parse_field_string(field_name.clone(), field_candidate).and_then({
    move |x| {
      x.parse()
...
fn parse_field_vec_string(
  ...
  parse_field_string(field_name, field_candidate).map(|x| {
    x.split(&separator)
      .map(|x| x.to_string())
      .collect::<Vec<String>>()
  })
...
pub fn line_to_machine_raw(line: &str) -> Result<EtcNixMachineRaw, AppError> {
  ...
  Ok(EtcNixMachineRaw {
    url: parse_field_string("url".to_string(), sub_parts.first())?,
    ...
    max_jobs: parse_field_u32("max_jobs".to_string(), sub_parts.get(3))?,
...
pub fn ssh_config_value_with_default(
  ...
  ssh_config_value(field, hostname).or_else({
    |e| match e {
...
pub fn url_with_ssh_config(s: &str) -> Result<Url, AppError> {
  let original = Url::parse(s).map_err(AppError::UrlParseError)?;
  let host = original
    .host_str()
    .ok_or(AppError::UrlHostnameMissingError(
  ...
  Url::parse(
    format!(
      ...
      ssh_config_value_with_default(
        "port",
        original.port().unwrap_or(22u16).to_string().as_str(),
        host,
      )?,
    )
    .as_str(),
...
pub fn parse(x: EtcNixMachineRaw) -> Result<Machine, AppError> {
  ...
  let host_str = url
    .host_str()
    .ok_or(AppError::MachinesEntryUrlHostnameMissingError(url_string))?;
  // SSH's default port if none is stated.
  let port = url.port().unwrap_or(22);
  Ok(Machine {
    url: url.clone(),
    platforms: x.platforms.clone(),
    user_private_key: user_private_key_loaded(
      host_str,
      &x.user_private_key_path,
    )
...
fn host_public_key_value(
  ...
  if public_key_base64 == "-" {
    ...
    ssh_keyscan(host_str, port).and_then(|entries| {
...
fn user_private_key_loaded(
  ...
  fs::read_to_string(private_key_path_infer(hostname, path)?)
    .map_err(|e| AppError::PrivateKeyFileReadError(e, path.to_string()))
}
