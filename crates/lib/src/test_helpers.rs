//! Shared fixtures and assertion helpers for the lib crate's
//! `#[cfg(test)]` modules.  Reachable only from test builds; the
//! whole module is gated on `#[cfg(test)]` from `lib.rs`.
//!
//! The clippy lints that ban `unwrap`/`expect`/`panic` are not in
//! force here — see workspace clippy.toml's `allow-*-in-tests`
//! switches, which cover code under `#[cfg(test)]`.

use crate::machine::Machine;
use crate::test::{AppTestContext, MachineTestContext, TestResult};
use url::Url;

pub fn create_test_machine(hostname: &str) -> Machine {
  Machine {
    url: Url::parse(&format!("ssh://builder@{}", hostname)).unwrap(),
    platforms: vec!["x86_64-linux".to_string()],
    user_private_key: "test_key_content".to_string(),
    user_private_key_path: "/tmp/test_key".to_string(),
    host_public_key: "test_host_public_key".to_string(),
    max_jobs: 1,
    speed_factor: 1,
    supported_features: vec![],
  }
}

pub fn create_test_context(hostname: &str) -> MachineTestContext {
  MachineTestContext {
    machine: create_test_machine(hostname),
    app_context: AppTestContext {},
  }
}

pub fn assert_test_passes(result: &TestResult, test_name: &str) {
  match result {
    TestResult::Pass(data) => {
      assert_eq!(data.test_name, test_name);
    }
    _ => panic!("Expected {} test to pass, but it didn't", test_name),
  }
}

pub fn assert_test_fails(result: &TestResult, test_name: &str) {
  match result {
    TestResult::Fail(data) => {
      assert_eq!(data.test_name, test_name);
    }
    _ => panic!("Expected {} test to fail, but it didn't", test_name),
  }
}
