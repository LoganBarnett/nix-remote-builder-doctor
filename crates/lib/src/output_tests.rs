...
mod tests {
  ...
  use crate::output::*;
  use crate::test::{
    AppTestContext, FailData, MachineTestContext, MachineTestResult, PassData,
    TestResult,
  };
  use url::Url;
  ...
  fn test_json_output_all_pass() {
    ...
    let results = vec![MachineTestResult {
    ...
    json_print(&results).expect("json_print should succeed");
