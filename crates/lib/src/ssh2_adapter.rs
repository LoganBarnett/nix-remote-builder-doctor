...
impl Ssh2 {
  pub fn new() -> Result<Ssh2, AppError> {
    Ok(Ssh2 {
      authenticated: false,
      session: Session::new().map_err(AppError::SshSessionCreateError)?,
      hostname: None,
      port: None,
    })
  }
...
impl Ssh for Ssh2 {
  ...
  fn run(&self, command: &str) -> Result<CommandOutput, AppError> {
    ...
    channel
      .wait_close()
      .map_err(AppError::SshChannelCloseError)?;
  ...
  fn check_host_key(&self) -> Result<bool, AppError> {
    ...
    let hostname = self.hostname.as_ref().ok_or_else(|| {
      AppError::SshHostKeyError(ssh2::Error::from_errno(
    ...
    let (host_key, _key_type) = self.session.host_key().ok_or_else(|| {
      AppError::SshHostKeyError(ssh2::Error::from_errno(
        ssh2::ErrorCode::Session(-1),
      ))
    })?;
