...
impl Test for DnsTest {
  fn test(&self, context: &MachineTestContext) -> Result<TestResult, AppError> {
    Ok(context.machine.url.host_str().map_or_else(
      || {
      ...
      |host_str| {
        ...
        match resolved_host(host_str) {
          ...
          Err(_) => TestResult::Fail(FailData {
            ...
            suggestion: "See if another host on the network can resolve \
                         this host, or check the host directly."
              .into(),
            test_name: "DNS".into(),
          }),
        }
...
mod tests {
  ...
  fn test_dns_resolution_invalid_host() {
    let context =
      create_test_context("this-host-definitely-does-not-exist.invalid");
    let dns_test = DnsTest {};
