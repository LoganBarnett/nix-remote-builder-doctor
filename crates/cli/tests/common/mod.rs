// Integration-test helper module.  Clippy's allow-unwrap-in-tests
// applies inside `#[test]` and `#[cfg(test)]`, but a `tests/common`
// helper module is compiled as ordinary code from the integration
// test binary's perspective and so the lint fires.  Suppress the
// panicking-variant lints here explicitly; these helpers are only
// reachable from #[test] entry points and panicking is the desired
// failure signal.
...
// `name`, `port`, and `ssh_key` are configured by tests but the helpers
// here don't read them yet; the structure documents the intended
// per-builder shape for the in-sandbox fixture.  Suppress dead_code
// rather than dropping the fields and losing that surface.
#[allow(dead_code)]
...
impl TestEnvironment {
  ...
  pub fn with_builder(mut self, builder: TestBuilder) -> Self {
    ...
    let pub_key =
      fs::read_to_string(&pub_key_path).expect("Failed to read public key");
...
"#,
  ...
  pub fn run_doctor(&self, args: &[&str]) -> String {
    ...
    cmd
      .env("PATH", self.temp_dir.path().join("bin"))
      .arg("--machines-file")
