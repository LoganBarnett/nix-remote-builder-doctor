{
  ...
  testNames = [
    ...
    {
      name = "local-to-remote-build";
      displayName = "Local To Remote Build";
    }
  ];

  # Generate a check script for a specific builder and test
  makeCheckScript = builder: test:
    pkgs.writeShellScriptBin "nix-remote-builder-doctor-check-${builder.name}-${test.name}" ''
      set -euo pipefail
